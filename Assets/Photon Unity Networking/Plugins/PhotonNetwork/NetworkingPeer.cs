...
internal class NetworkingPeer : LoadbalancingPeer, IPhotonPeerListener
{
            ...
            case OperationCode.Authenticate:
                {
                    ...
                    if (operationResponse.ReturnCode != 0)
                    {
                        ...
                        {
                            if (PhotonNetwork.logLevel >= PhotonLogLevel.Informational)
                                Debug.LogWarning(string.Format("Currently, the limit of users is reached for this missionTitle. Try again later. Disconnecting"));
        ...
        switch (photonEvent.Code)
        {
            ...
            case EventCode.Join:
                ...
                if (actorNr == this.mLocalActor.ID)
                {
                    ...
                    SendMonoMessage(PhotonNetworkingMessage.OnJoinedRoom); //Always send UpdatePlayerList
    ...
    ///
    /// This is sent as event (code: 200) which will contain a senderPlayer (origin of this RPC).
    ...
    private void OnSerializeRead(Hashtable data, PhotonPlayer sender, int networkTime, short correctPrefix)
    {
        ...
        if (sender.ID != view.ownerId)
        {
            if (!view.isSceneView || !sender.isMasterClient)
            {
                ...
                Debug.Log("Adjusting owner to senderPlayer of updates. From: " + view.ownerId + " to: " + sender.ID);
